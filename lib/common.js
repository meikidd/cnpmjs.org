...
exports.setWnpmDownloadURL = function (pkg, ctx, host) {
  if (pkg.dist) {
    host = host || config.registryHost || ctx.host;
    var path = url.parse(pkg.dist.tarball).path;
    pkg.dist.tarball = util.format('%s://%s/wnpm-download/%s/%s%s',
      ctx.protocol, host, pkg.name, pkg.version, path);
  }
};
